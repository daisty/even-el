...
}elseif($_GET['do']=='checkvc'){
	$data='';
	foreach($_GET as $k=>$get){
		if($k=='do')continue;
		$data.="$k=$get&";
	}
	$url="http://check.ptlogin2.qq.com/check?".$data;
	$get=get_curl($url);
	exit($get);
}elseif($_GET['do']=='cap_union_prehandle'){
	$url="http://captcha.qq.com/cap_union_prehandle?".$prams;
	$get=get_curl($url);
	exit($get);
...
//$get=preg_replace('!"//tajs.qq.com(.*?)"!i','',$get);
